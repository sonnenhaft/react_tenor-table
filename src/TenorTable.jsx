...
export const TenorTable = () => {
  ...
  const editNumberFromContentEditable = item => (e) => {
  ...
  return <Table>
      ...
      { meta.map(({ format, key, style, editable }) => {
        ...
        return editable
          ? <td style={ { ...style, cursor: 'pencil', background: '#dadada' } } key={ key } contentEditable
                onInput={ editNumberFromContentEditable(item) }>
            { format(item[key]) }
          </td>
    ...
    <tr>{ meta.map(({ total, format, key }) => {
      return <th key={ key }>{ total && total !== 'avg' ? total : format(values
        .reduce((sum, item) => {
          return sum + (item.hasOwnProperty('tempTotal' + key) ? item['tempTotal' + key] : item[key])
        }, 0) / (total === 'avg' ? values.length : 1)) }</th>
    }) }</tr>
  </Table>;
};